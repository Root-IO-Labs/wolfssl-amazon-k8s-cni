#!/bin/bash
#
# Build script for AWS VPC CNI (aws-node) v1.21.1 FIPS-enabled image
#
# Usage: ./build.sh [OPTIONS]
#
# Options:
#   --tag TAG          Custom image tag (default: amazon-k8s-cni-fips:v1.21.1-ubuntu-22.04)
...
DEFAULT_TAG="amazon-k8s-cni-fips:v1.21.1-ubuntu-22.04"
...
Examples:
    ...
    $0 --tag my-registry.com/aws-cni-fips:v1.21.1
...
Build Time:
    - Total: ~50-60 minutes
    - Longest stage: golang-fips/go toolchain (~30-40 min)

Components Built:
    - aws-k8s-agent (IPAM daemon)
    - aws-cni (CNI plugin)
    - egress-cni (Egress plugin)
    - grpc-health-probe (Health check)
    - aws-vpc-cni (Entrypoint)
...
echo ""
echo "========================================"
echo "AWS VPC CNI (aws-node) FIPS Build"
echo "========================================"
echo ""
...
log_success "entrypoint.sh found"

# Check if openssl-wolfprov.cnf exists
if [ ! -f "openssl-wolfprov.cnf" ]; then
    log_error "openssl-wolfprov.cnf not found!"
    exit 1
fi
log_success "openssl-wolfprov.cnf found"

# Check if fips-startup-check.c exists
if [ ! -f "fips-startup-check.c" ]; then
    log_error "fips-startup-check.c not found!"
    exit 1
fi
log_success "fips-startup-check.c found"
...
echo "  4. golang-fips/go toolchain (longest stage ~30-40 min)"
echo "  5. aws-vpc-cni v1.21.1 components (5 binaries)"
...
if docker buildx build \
    ...
    echo "  docker run --rm --entrypoint=/bin/bash ${IMAGE_TAG} -c 'openssl list -providers'"
    echo ""
    echo "  # Test FIPS-approved algorithm"
    echo "  docker run --rm --entrypoint=/bin/bash ${IMAGE_TAG} -c 'echo test | openssl dgst -sha256'"
    echo ""
    echo "  # Check for non-FIPS crypto libraries (should be empty)"
    echo "  docker run --rm --entrypoint=/bin/bash ${IMAGE_TAG} -c 'find /usr/lib /lib -name \"libgnutls*\"'"
    echo ""
    echo "  # Run FIPS validation only (skip daemon startup)"
    echo "  docker run --rm --entrypoint=/app/entrypoint.sh ${IMAGE_TAG} /bin/bash"
    echo ""
    echo "  # Test daemon startup (requires --net=host --privileged)"
    echo "  docker run --rm --net=host --privileged \\"
    echo "    -e NODE_NAME=test-node \\"
    echo "    -v /var/run/aws-node:/var/run/aws-node \\"
    echo "    ${IMAGE_TAG}"
    echo ""

    log_info "Kubernetes deployment:"
    echo "  See kubernetes-daemonset.yaml for example DaemonSet configuration"
    echo ""
