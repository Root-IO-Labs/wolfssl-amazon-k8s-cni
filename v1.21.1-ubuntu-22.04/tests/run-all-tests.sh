#!/bin/bash
################################################################################
# AWS VPC CNI (aws-node) v1.21.1 - Master Test Runner
#
...
#
# Example:
#   ./tests/run-all-tests.sh amazon-k8s-cni-fips:v1.21.1-ubuntu-22.04
#
# Test Suites:
#   1. Quick Test (12 checks)
#   2. FIPS Compliance Verification (62 checks)
#   3. Daemon Functionality Tests (30 checks)
#   4. Non-FIPS Algorithm Blocking (11 checks)
#   5. Cryptographic Path Validation (34 checks)
#
# Total Checks: ~149
...
print_banner() {
    echo ""
    echo "================================================================================"
    echo -e "${BOLD}${CYAN}AWS VPC CNI (aws-node) v1.21.1 - Master Test Runner${NC}"
    echo "================================================================================"
    echo ""
...
################################################################################
# Test Suite 3: Daemon Functionality Tests
################################################################################

run_test_suite 3 \
    "Daemon Functionality" \
    "test-cni-daemon-functionality.sh"
...
if [ $FAILED_SUITES -eq 0 ]; then
    ...
    echo "  • All 5 daemon binaries using FIPS cryptography"
    echo "  • Daemon functionality verified"
    echo "  • Network tools operational"
    echo ""
    echo "The image is READY FOR PRODUCTION USE with full FIPS 140-3 compliance."
    echo ""
    echo "Next Steps:"
    echo "  1. Deploy to Kubernetes: kubectl apply -f kubernetes-daemonset.yaml"
    echo "  2. Monitor logs: kubectl logs -n kube-system -l k8s-app=aws-node"
    echo "  3. Verify IPAM: kubectl exec -n kube-system <pod> -- curl http://localhost:61679/v1/enis"
    echo ""
    exit 0
else
    echo "================================================================================"
    ...
    echo "     - Binaries not compiled with golang-fips/go"
    ...
    echo "  ./tests/test-cni-daemon-functionality.sh"
