...
################################################################################
# OpenSCAP Internal Container Scan for AWS VPC CNI (aws-node)
...
# Configuration
IMAGE_NAME="${1:-amazon-k8s-cni:v1.21.1-ubuntu-22.04-fips}"
CONTAINER_NAME="aws-node-internal-scan-$$"
...
echo ""
echo "=========================================="
echo "OpenSCAP Internal Container Scan - AWS VPC CNI"
...
# AWS VPC CNI container - start with tail to keep it alive for scanning
# Override ENTRYPOINT to bypass FIPS validation and aws-node startup during scanning
...
################################################################################
# Verify AWS VPC CNI Installation
################################################################################

echo ""
echo -e "${BLUE}[INFO]${NC} Verifying AWS VPC CNI installation..."

# Check for aws-k8s-agent binary (main IPAM daemon)
if docker exec --user root "$CONTAINER_NAME" test -f /app/aws-k8s-agent; then
    echo -e "${GREEN}✓${NC} aws-k8s-agent binary found"
fi

# Check for aws-cni plugin
if docker exec --user root "$CONTAINER_NAME" test -f /app/aws-cni; then
    echo -e "${GREEN}✓${NC} aws-cni plugin found"
fi

# Check for egress-cni plugin
if docker exec --user root "$CONTAINER_NAME" test -f /app/egress-cni; then
    echo -e "${GREEN}✓${NC} egress-cni plugin found"
fi

# Check for aws-vpc-cni entrypoint
if docker exec --user root "$CONTAINER_NAME" test -f /app/aws-vpc-cni; then
    echo -e "${GREEN}✓${NC} aws-vpc-cni entrypoint found"
fi

# Detect version
CNI_VERSION=$(docker exec --user root "$CONTAINER_NAME" sh -c '/app/aws-k8s-agent --version 2>&1 | grep -o "v[0-9]\+\.[0-9]\+\.[0-9]\+" || echo "v1.21.1"' | head -1 || echo "v1.21.1")
echo -e "${GREEN}✓${NC} AWS VPC CNI version: $CNI_VERSION"
...
STIG_REPORT_XML="$REPORT_DIR/aws-node-internal-stig-${TIMESTAMP}.xml"
STIG_REPORT_HTML="$REPORT_DIR/aws-node-internal-stig-${TIMESTAMP}.html"
...
CIS_REPORT_XML="$REPORT_DIR/aws-node-internal-cis-${TIMESTAMP}.xml"
CIS_REPORT_HTML="$REPORT_DIR/aws-node-internal-cis-${TIMESTAMP}.html"
...
echo ""
echo "=========================================="
echo "Python-Specific Security Checks"
echo "=========================================="
echo ""

echo -e "${BLUE}[INFO]${NC} Running Python security verification..."

docker exec --user root "$CONTAINER_NAME" bash -c '
# Check FIPS compliance
echo "FIPS Compliance:"
if python3 -c "import ssl; print(ssl.OPENSSL_VERSION)" 2>/dev/null | grep -q "OpenSSL 3"; then
    echo "  ✓ Python linked to OpenSSL 3"
fi

# Check blocked packages
if [ -f /etc/pip-fips-blocklist.txt ]; then
    BLOCKED_COUNT=$(grep -cv "^#\|^$" /etc/pip-fips-blocklist.txt)
    echo "  ✓ pip blocklist active: $BLOCKED_COUNT packages blocked"
fi

# Check pip wrapper
if [ -f /usr/local/bin/pip3.real ]; then
    echo "  ✓ pip wrapper installed (blocks non-FIPS packages)"
fi

# Check build tools removed
if ! command -v gcc &>/dev/null; then
    echo "  ✓ Build tools removed (cannot build from source)"
fi
